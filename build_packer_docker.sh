...
docker image rmi \
    $DOCKER_USERNAME/$PACKAGE_NAME:$PACKAGE_VERSION-$IMAGE_NAME-$(date -u '+%Y%m%d') -f  || true
docker image rmi \
    $DOCKER_USERNAME/$PACKAGE_NAME:$PACKAGE_VERSION-$IMAGE_NAME -f  || true
docker container rm $PACKAGE_NAME-$PACKAGE_VERSION-$IMAGE_NAME-$BUILD_DATE -f || true
...
# push images
docker push $DOCKER_USERNAME/$PACKAGE_NAME:$PACKAGE_VERSION-$IMAGE_NAME-$(date -u '+%Y%m%d')
docker push $DOCKER_USERNAME/$PACKAGE_NAME:$PACKAGE_VERSION-$IMAGE_NAME
